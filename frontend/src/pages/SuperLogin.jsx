...
export default function SuperLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!data.isSuperUser) { setError('This portal is for Superuser accounts only.'); return; }
      localStorage.setItem('token', data.token);
      localStorage.setItem('user', JSON.stringify({ email: data.email, isSuperUser: true, name: data.name || null }));
      nav('/rt-super', { replace: true });
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-950 px-4">
      <div className="mb-8 text-center">
        <div className="w-14 h-14 bg-purple-600 rounded-xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-purple-900/40">
          <ShieldCheck size={28} className="text-white" />
        </div>
        <h1 className="text-white font-bold text-2xl tracking-tight">Superuser Portal</h1>
        <p className="text-gray-400 text-sm mt-1">Highest-privilege access — authorized personnel only</p>
      </div>
      ...
      <div className="w-full max-w-sm">
        ...
        {mfaStep && (
          <div className="card p-8">
            <div className="flex flex-col items-center mb-6 text-center">
              <div className="w-14 h-14 bg-purple-600/10 rounded-2xl flex items-center justify-center mb-3">
            ...
            <form onSubmit={handleMfaSubmit} className="space-y-4">
              ...
              <button type="submit" disabled={loading}
                className="w-full py-2.5 px-4 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-lg transition disabled:opacity-60">
                {loading ? 'Verifying…' : 'Verify & Sign In'}
              </button>
              <button type="button" onClick={() => { setMfaStep(false); setMfaToken(''); setMfaCode(''); setError(''); }}
                className="w-full text-center text-sm text-gray-400 hover:text-gray-300 transition">← Back to login</button>
            </form>
          </div>
        )}
        ...
        <p className="text-center text-xs text-gray-600 mt-6">
          <button onClick={() => nav('/rt-admin/login')} className="text-gray-500 hover:text-gray-300 underline transition">
            ← Admin Portal
          </button>
        </p>
      </div>
